...
export default async function handler(req, res) {
  ...
  const { textChars = 0, imageCount = 0, useWishlistSummary = false, wishlistItemCount = 0 } = req.body || {};
  res.status(200).json(estimateStyleGeneration({ textChars, imageCount, useWishlistSummary, wishlistItemCount }));
}
