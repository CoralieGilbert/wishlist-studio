...
function outfitCard(o){const imgs=outfitCoverImages(o).slice(0,4);const n=(o.itemIds||[]).length;return `<article class="outfit-card"><button style="border:0;padding:0;background:none;width:100%;text-align:left" onclick="openOutfitDetail('${o.id}')"><div class="outfit-cover ${imgs.length<=1?'one':''}">${imgs.length?imgs.map(img=>`<img src="${img}" alt="">`).join(''):'<div></div>'}<span class="outfit-cover-shade"></span><span class="outfit-cover-copy"><span>TENUE · ${n} pièce${n>1?'s':''}</span><strong>${esc(o.name||'Tenue sans titre')}</strong></span></div><div class="outfit-copy"><p>${esc((o.tags||[]).join(' · '))}</p></div></button><div class="outfit-copy" style="padding-top:0"><div class="outfit-actions"><button onclick="openOutfitDetail('${o.id}')">Voir</button><button onclick="openCollectionPicker('${o.id}')">+ Collection</button><button onclick="openOutfitEditor('${o.id}')">Modifier</button></div></div></article>`}
...
function renderShoppingModal(){
  ...
  document.getElementById('shoppingModalBody').innerHTML=`
    ...
    <label><span>Budget (optionnel)</span><input id="shopBudget" type="number" min="1" step="1" placeholder="laisser vide = pas de limite" oninput="onShoppingSliderChange()"></label>
...
async function runShoppingGenerate(){
  ...
  const query=(document.getElementById('shopQuery')?.value||'').trim();
